...
class UartBridgeAma1Node : public rclcpp::Node {
public:
  UartBridgeAma1Node()
  : Node("uart_bridge_ama1_node"), fd_(-1), stop_(false)
  {
    // === Params ===
    port_      = this->declare_parameter<std::string>("port", "/dev/ttyAMA1"); // AMA1 기본
    baud_      = this->declare_parameter<int>("baud", 115200);
    newline_   = this->declare_parameter<std::string>("newline", "\r\n");      // CRLF 기본
    rx_topic_  = this->declare_parameter<std::string>("rx_topic", "/stm32/uart/rx");
    tx_topic_  = this->declare_parameter<std::string>("tx_topic", "/stm32/uart/tx");
    ...
    sub_tx_ = this->create_subscription<std_msgs::msg::String>(
      tx_topic_, 10, std::bind(&UartBridgeAma1Node::on_tx_msg, this, std::placeholders::_1));

    // === Open serial (exclusive) ===
    ...
    // === RX thread ===
    rx_thread_ = std::thread(&UartBridgeAma1Node::rx_loop, this);

    // === Shutdown hook ===
    rclcpp::on_shutdown([this](){ this->graceful_stop(); });
  }

  ~UartBridgeAma1Node() override { graceful_stop(); }

private:
  // ---------- utils ----------
  static std::string sanitize_printable(const std::string &s, size_t max_len = 200) {
    std::string out; out.reserve(std::min(s.size(), max_len));
  ...
  static std::string hex_dump(const uint8_t* data, size_t len, size_t max_len = 256) {
    std::ostringstream oss; size_t n = std::min(len, max_len);
  ...
  int open_serial(const char* dev, int baud) {
...
#ifdef TIOCEXCL
    ioctl(fd, TIOCEXCL); // 포트 독점 (다른 프로세스와 충돌 방지)
#endif
    ...
    tio.c_iflag &= ~(IXON | IXOFF | IXANY);    // XON/XOFF off
#ifdef CRTSCTS
    tio.c_cflag &= ~CRTSCTS;              // HW flow off
#endif
    // select() 기반 폴링: 비블로킹 성격을 위해 VMIN/VTIME 0
    tio.c_cc[VMIN]  = 0;
    tio.c_cc[VTIME] = 0;
  ...
  void on_tx_msg(const std_msgs::msg::String::SharedPtr msg) {
    ...
    if (!newline_.empty()) {
      if (newline_ == "\r\n") {
        ...
        if (data.empty() || data.back() != '\r') data += "\r";
      }
    }
    ...
    if (left == 0) tcdrain(fd_); // 전송 완료까지 대기 (짧은 라인일 때 유용)
  }
  ...
  void rx_loop() {
    ...
    while (!stop_.load()) {
      ...
      struct timeval tv; tv.tv_sec = 0; tv.tv_usec = static_cast<suseconds_t>(poll_ms_) * 1000;

      int rv = ::select(fd_ + 1, &rfds, nullptr, nullptr, &tv);
      if (rv < 0) {
        if (errno == EINTR) continue;
        if (stop_.load()) break;
        RCLCPP_WARN(get_logger(), "select: %s", std::strerror(errno));
        continue;
      }
      if (rv == 0 || !FD_ISSET(fd_, &rfds)) continue;
      ...
      if (n > 0 && log_bytes_) {
        RCLCPP_DEBUG(get_logger(), "[RX bytes] %zd bytes: %s", n, hex_dump(tmp.data(), tmp.size()).c_str());
      }

      for (uint8_t c : tmp) {
        if (c == '\n') {
          if (!linebuf.empty() && linebuf.back() == '\r') linebuf.pop_back(); // CRLF 처리
          if (log_lines_) RCLCPP_INFO(get_logger(), "[RX line] \"%s\"", sanitize_printable(linebuf).c_str());
          std_msgs::msg::String msg; msg.data = linebuf;
          pub_rx_->publish(msg);
          linebuf.clear();
        } else {
  ...
  void graceful_stop() {
    ...
    RCLCPP_INFO(get_logger(), "Stopping uart_bridge_ama1_node...");
    { std::lock_guard<std::mutex> lk(tx_mtx_);
...
int main(int argc, char** argv) {
  ...
  try {
    auto node = std::make_shared<UartBridgeAma1Node>();
    rclcpp::spin(node);
