...
public:
  UartBridgeAma1Node() : Node("uart_bridge_ama1_node"), fd_(-1), stop_(false) {
    // ★ 포트/토픽/개행 파라미터화 (STM과 맞춤)
    port_      = this->declare_parameter<std::string>("port", "/dev/ttyAMA1");
    baud_      = this->declare_parameter<int>("baud", 115200);
    newline_   = this->declare_parameter<std::string>("newline", "\r\n"); // TX 시 붙임
    rx_topic_  = this->declare_parameter<std::string>("rx_topic", "/stm/rx");
    tx_topic_  = this->declare_parameter<std::string>("tx_topic", "/stm/tx");
...
private:
  ...
  void rx_loop() {
    ...
    while (!stop_.load()) {
      ...
      struct timeval tv;
      tv.tv_sec  = 0;
      tv.tv_usec = static_cast<suseconds_t>(poll_ms_) * 1000;
